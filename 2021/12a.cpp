...
int main(int argc, char** argv)
{
	...
	stack.push_back(Frame{ { "start" }, &*paths.find("start") });
	...
	while (!stack.empty())
	{
		...
		Frame next{ last.visited, &*paths.find(loc) };
		if (!isupper(loc[0]))
		{
			if (std::find(last.visited.begin(), last.visited.end(), loc) != last.visited.end())
			{
				continue;
			}
